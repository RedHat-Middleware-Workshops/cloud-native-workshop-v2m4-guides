...
[source,sh,role="copypaste"]
----
oc label dc/inventory app.kubernetes.io/part-of=inventory app.openshift.io/runtime=quarkus --overwrite && \
...
Wait for that command to report *replication controller cart-service-1_ successfully rolled out* before continuing.

And label the items with proper icons:

[source,sh,role="copypaste"]
----
oc label dc/cart-service app.kubernetes.io/part-of=cart app.openshift.io/runtime=quarkus --overwrite && \
oc label dc/datagrid-service app.kubernetes.io/part-of=cart app.openshift.io/runtime=datagrid --overwrite && \
oc annotate dc/cart-service app.openshift.io/connects-to=catalog,datagrid-service --overwrite && \
oc annotate dc/cart-service app.openshift.io/vcs-ref=ocp-4.4 --overwrite
----
...
And then access the http://cart-service-{{ USER_ID }}-cloudnativeapps.{{ ROUTE_SUBDOMAIN }}/swagger-ui[Cart Swagger UI^]:
...
Wait for that command to report *replication controller order-1_ successfully rolled out* before continuing.
