...
Open a new brower with the {{ CONSOLE_URL }}[OpenShift web console^]:
...
Now let's deploy the application itself. Open the `application.properties` in the *src/main/resources* directory. Append the following configurations.

[source,properties,role="copypaste"]
----
%prod.quarkus.openshift.deployment-kind=Deployment
%prod.quarkus.container-image.group={{ USER_ID }}-cloudnativeapps
...
[NOTE]
====
In case you can't access the web page, please check if your web browser (e.g., Chrome) redirects to the page using `HTTPS` protocol. You must access the page using `HTTP` protocol.
====
...
quarkus.openshift.route.expose=true<5>
...
[source,sh,role="copypaste"]
----
oc rollout status -w deployment/cart
----
...
oc annotate deployment/cart app.openshift.io/vcs-ref=ocp-4.14 --overwrite
----
...
[source,sh,role="copypaste"]
----
oc new-app -n {{ USER_ID }}-cloudnativeapps  --docker-image quay.io/openshiftlabs/ccn-mongo:4.0 --name=order-database
----
...
[source,sh,role="copypaste"]
----
oc rollout status -w deployment/order
----

Wait for that command to report *deployment "order" successfully rolled out* before continuing.
...
oc annotate deployment/order app.openshift.io/vcs-ref=ocp-4.14 --overwrite
----
