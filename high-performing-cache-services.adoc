...
oc label deployments/catalog app.kubernetes.io/part-of=catalog app.openshift.io/runtime=spring --overwrite && \
oc label dc/catalog-database app.kubernetes.io/part-of=catalog app.openshift.io/runtime=postgresql --overwrite && \
oc annotate deployments/catalog app.openshift.io/connects-to=inventory,catalog-database --overwrite && \
oc annotate deployments/catalog app.openshift.io/vcs-uri=https://github.com/RedHat-Middleware-Workshops/cloud-native-workshop-v2m4-labs.git --overwrite && \
oc annotate deployments/catalog app.openshift.io/vcs-ref=ocp-4.5 --overwrite
----
...
----
<1> So our `ShoppingCart` has a list (`repeated`) of `ShoppingCartItem`
<2> Each `ShoppingCartItem` has a `Product`

But we haven't defined the `Product` yet. Add this code below the `//TODO ADD Product` marker:
...
oc label deployments/datagrid-service app.kubernetes.io/part-of=cart app.openshift.io/runtime=datagrid --overwrite && \
...
oc label deployments/order-database app.kubernetes.io/part-of=order app.openshift.io/runtime=mongodb --overwrite && \
