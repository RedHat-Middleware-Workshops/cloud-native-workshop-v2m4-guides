...
First, open a new brower with the {{ CONSOLE_URL%7D%7D[OpenShift web console,window=_blank]
...
using a BOM provided by Red Hat as part of the http://snowdrop.me/[Snowdrop,window=_blank] project.
...
[source,sh,role="copypaste"]
----
mvn clean package spring-boot:repackage -DskipTests -f $CHE_PROJECTS_ROOT/cloud-native-workshop-v2m1-labs/catalog-service
----
...
Go to `Networking > Routes` in {{ CONSOLE_URL%7D%7D[OpenShift web console,window=_blank] and click on the route
URL of `coolstore-ui`:
