...
[source,console]
----
2020-03-10 00:55:12,598 INFO  [io.agr.pool] (main) Datasource '<default>': Initial size smaller than min. Connections will be created when necessary
2020-03-10 00:55:12,887 INFO  [io.quarkus] (main) inventory 1.0-SNAPSHOT (running on Quarkus 1.2.1.Final) started in 3.166s. Listening on: http://0.0.0.0:8080
2020-03-10 00:55:12,890 INFO  [io.quarkus] (main) Profile dev activated. Live Coding activated.
2020-03-10 00:55:12,890 INFO  [io.quarkus] (main) Installed features: [agroal, cdi, hibernate-orm, hibernate-orm-panache, jdbc-h2, narayana-jta, resteasy, resteasy-jsonb, smallrye-health]
----
...
Let's switch to {{CONSOLE_URL}}/add/ns/{{USER_ID}}-cloudnativeapps[Developer Console^].
...
Let's take a look at our `Product.java` in `cart-service/src/main/java/com/redhat/cloudnative/model`:
...
_WEB-UI Service_ serves a frontend based on https://angularjs.org/[AngularJS^] and http://patternfly.org/[PatternFly^] running in a https://access.redhat.com/documentation/en/openshift-container-platform/3.3/paged/using-images/chapter-2-source-to-image-s2i[Node.js] container. https://developers.redhat.com/products/rhoar[Red Hat OpenShift Application Runtimes] includes *Node.js* support in enterprise prouction environment.
...
In the end, we optimized *data transaction performance* of the shopping cart service thru integrating with a *Red Hat Data Grid* to increase end users’(customers) satification. *Congratulations!*
