...
In VS Code, expand *payment-service* directory.
...
Let's add Maven Dependencies using Quarkus Kafka extensions in VS Code Terminal:

[source,sh,role="copypaste"]
----
mvn quarkus:add-extension -Dextensions="messaging-kafka" -f $PROJECT_SOURCE/payment-service
----
...
Append the following configureations for the OpenShift Deployment in the `application.properties` as well.

[source,sh,role="copypaste"]
----
quarkus.openshift.deployment-kind=Deployment<6>
quarkus.container-image.group={{ USER_ID }}-cloudnativeapps<7>
quarkus.container-image.registry=image-registry.openshift-image-registry.svc:5000<8>
----

Build and deploy the project using using the OpenShift extension, which will use the maven plugin to deploy via VS Code Terminal:
...
[source,sh,role="copypaste"]
----
oc rollout status -w deployment/payment
----

Wait for that command to report *deployment "payment" successfully rolled out* before continuing.

And label the items with proper icons:

[source,sh,role="copypaste"]
----
oc label deployment/payment app.kubernetes.io/part-of=payment --overwrite && \
oc annotate deployment/payment app.openshift.io/connects-to=my-cluster --overwrite && \
oc annotate deployment/payment app.openshift.io/vcs-ref=ocp-4.14 --overwrite
----
...
In order to test the payment application, go to the {{ CONSOLE_URL }}/k8s/ns/{{ USER_ID }}-cloudnativeapps/pods/my-cluster-kafka-0[Pod detail^] and click on the `my-cluster-kafka-0` pod. We will watch the Kafka topic via a CLI to confirm the messages are being sent/received in Kafka. Click on the *Terminal* tab in OpenShift (not in VS Code!) then execute the following command:
...
Now, back in *VS Code*, open a new Terminal.
...
[source,sh,role="copypaste"]
----
mvn quarkus:add-extension -Dextensions="messaging-kafka" -f $PROJECT_SOURCE/cart-service
----
...
Re-package the _cart service_ using the following command, which will use the maven plugin to deploy via VS Code Terminal:

[source,sh,role="copypaste"]
----
mvn clean package -DskipTests -DskipTests -f $PROJECT_SOURCE/cart-service && \
oc label deployment/cart app.kubernetes.io/part-of=cart --overwrite &&  \
oc annotate deployment/cart app.openshift.io/connects-to=my-cluster,datagrid-service --overwrite
----
...
[source,sh,role="copypaste"]
----
mvn quarkus:add-extension -Dextensions="messaging-kafka" -f $PROJECT_SOURCE/order-service
----
...
Re-package the order service using the following command, which will use the maven plugin to deploy via VS Code Terminal:
...
oc annotate deployment/order app.openshift.io/connects-to=my-cluster,order-database --overwrite
----
