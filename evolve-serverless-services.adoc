...
This shows that our process is taking around 67MB of memory (https://en.wikipedia.org/wiki/Resident_set_size[Resident Set
...
<1> Clones the source code of the backend application from a git repository when (_git-repo_ git resource) referred to as _api-repo_
...
<3> The application image is pushed to an image registry specified by the _image_ resource to _api-image_
...
<2> Builds the container image of backend using the  _buildah_ clustertask that uses https://buildah.io/[Buildah^] to build the image
<3> The application image is pushed to an image registry specified by the _image_ resource to _ui-image_
<4> The new backend application image is deployed on OpenShift using the _apply-manifests_ and _update-deployment_ tasks.
