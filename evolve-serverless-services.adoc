...
install it via the _Knative Eventing Operator_ in {{ CONSOLE_URL%7D%7D[OpenShift web console,window=_blank].
...
{{ CONSOLE_URL%7D%7D[OpenShift web console,window=_blank] then confirm if _payment service_ is up automatically.
It’s `MAGIC!!`
...
You should be able to see the deployment in the {{ CONSOLE_URL%7D%7D[OpenShift web console,window=_blank].
...
In the {{ CONSOLE_URL%7D%7D[OpenShift web console,window=_blank], you can click on _Add > Import YAML_ at the top
...
{{ CONSOLE_URL%7D%7D[OpenShift web console,window=_blank] and Tekton CLI. In this tutorial, you will trigger the
...
pipeline resources via the {{ CONSOLE_URL%7D%7D[OpenShift web console,window=_blank] via `Add → Import YAML`:
...
resources via the {{ CONSOLE_URL%7D%7D[OpenShift web console,window=_blank] via `Add → Import YAML`. Replace your
...
Create the above pipeline resources via the {{ CONSOLE_URL%7D%7D[OpenShift web console,window=_blank] via
`Add → Import YAML`.
