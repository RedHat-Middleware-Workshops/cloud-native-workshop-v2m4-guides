...
INFO  [io.quarkus] (main) getting-started 1.0-SNAPSHOT native (powered by Quarkus xx.xx.xx) started in 0.012s. Listening on: http://0.0.0.0:8080
INFO  [io.quarkus] (main) Profile prod activated. 
INFO  [io.quarkus] (main) Installed features: [cdi, resteasy]
----

That’s *12 milliseconds* to start up. The start-up time might be different in your environment.
...
----
    PID   RSS COMMAND
   8208 35304 /tmp/hello/getting-started/target/getting-started-1.0.0-SNAPSHOT-runner
----

This shows that our process is taking around 35MB of memory (https://en.wikipedia.org/wiki/Resident_set_size[Resident Set
...
The _-Pnative_ argument selects the native maven profile which invokes the _Graal compiler_. It will take a few minutes(up to 10 mins) to complete a native binary build as well as deploying a new knative service. After successful creation of the service we should see a *Knative Service*(_KSVC_) and *Revision*(_REV_) in the {{ CONSOLE_URL }}/topology/ns/{{ USER_ID }}-cloudnativeapps[Topology View]:
...
buildah-v0-22-0            Buildah task builds...   3 hours ago
----
