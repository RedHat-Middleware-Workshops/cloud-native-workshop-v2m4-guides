...
And the following `api-repo` PipelineResource defines the git repository for the backend application in `YAML` editor then click on *Create*:
...
And the following `api-image` PipelineResource defines the OpenShift internal image registry for the backend image to be pushed to in `YAML` editor then click on *Create*:
...
Let's run the pipeline to deploy `vote-ui` application. Go back to {{ CONSOLE_URL }}/k8s/ns/{{ USER_ID }}-cloudnative-pipeline/tekton.dev~%7E1beta1%7EPipeline[OpenShift Pipelines^] and click on *Start*. Then input the parameter and select resources as below then Click on *Start*:
