...
2020-04-06 13:48:33,523 INFO  [io.quarkus] (main) getting-started 1.0-SNAPSHOT (powered by Quarkus xx.xx.xx) started in 0.022s. Listening on: http://0.0.0.0:8080
...
That’s *18 milliseconds* to start up. The start-up time might be different in your environment.
...
----
    PID   RSS COMMAND
   PID   RSS COMMAND
   2710 60424 /tmp/hello/getting-started/target/getting-started-1.0-SNAPSHOT-runner
----

This shows that our process is taking around `60 MB` of memory (https://en.wikipedia.org/wiki/Resident_set_size[Resident Set
...
Let's edit labels to add _Quarkus_ icon. Click on payment *REV* then select *Edit Labels* in _Actions_ drop box:
...
Copy the following `petclinic-deploy-pipeline` Pipeline in `YAML` editor then click on *Create*:
