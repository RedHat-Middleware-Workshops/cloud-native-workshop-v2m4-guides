...
`GraalVM` is already installed for you. Inspect the value of the `GRAALVM_HOME` variable in the VS Code Terminal
with:
...
Let’s find out why Quarkus calls itself _SuperSonic Subatomic Subatomic Java_. Let’s build a sample app. In VS Code Terminal, run this command:

[source,sh,role="copypaste"]
----
mkdir /tmp/hello && cd /tmp/hello && \
mvn com.redhat.quarkus.platform:quarkus-maven-plugin:3.2.6.Final-redhat-00002:create \
    -DprojectGroupId=com.redhat.quarkus.platform \
    -DprojectArtifactId=getting-started \
    -DplatformGroupId=com.redhat.quarkus.platform \
    -DplatformVersion=3.2.6.Final-redhat-00002 \
...
[2/7] Performing analysis...  [************]                                                            (77.5s @ 1.27GB)
  10,503 (88.48%) of 11,871 classes reachable
  15,026 (57.48%) of 26,143 fields reachable
  51,943 (55.35%) of 93,845 methods reachable
     536 classes,   109 fields, and 2,641 methods registered for reflection
      63 classes,    68 fields, and    55 methods registered for JNI access
       4 native libraries: dl, pthread, rt, z
[3/7] Building universe...                                                                               (8.1s @ 1.67GB)
[4/7] Parsing methods...      [***]                                                                      (8.4s @ 1.52GB)
[5/7] Inlining methods...     [***]                                                                      (4.2s @ 1.50GB)
[6/7] Compiling methods...    [*******]                                                                 (58.5s @ 0.75GB)
[7/7] Creating image...                                                                                  (5.0s @ 1.04GB)
  19.55MB (47.00%) for code area:    33,184 compilation units
  21.71MB (52.22%) for image heap:  283,756 objects and 8 resources
 332.12KB ( 0.78%) for other data
  41.58MB in total
------------------------------------------------------------------------------------------------------------------------
Top 10 packages in code area:                               Top 10 object types in image heap:
   1.62MB sun.security.ssl                                     4.33MB byte[] for code metadata
 975.20KB java.util                                            2.59MB java.lang.String
 725.94KB java.lang.invoke                                     2.48MB java.lang.Class
 717.62KB com.sun.crypto.provider                              2.39MB byte[] for general heap data
 482.46KB java.lang                                            2.00MB byte[] for java.lang.String
 450.52KB sun.security.x509                                  902.60KB com.oracle.svm.core.hub.DynamicHubCompanion
 405.35KB io.netty.buffer                                    745.13KB java.util.HashMap$Node
 384.44KB java.io                                            564.84KB byte[] for reflection metadata
 381.04KB java.util.concurrent                               519.83KB java.lang.String[]
 346.62KB io.netty.handler.codec.http2                       390.93KB byte[] for embedded resources
  12.92MB for 391 more packages                                4.42MB for 2555 more object types
------------------------------------------------------------------------------------------------------------------------
                        8.8s (5.0% of total time) in 47 GCs | Peak RSS: 3.37GB | CPU load: 2.40
------------------------------------------------------------------------------------------------------------------------
Produced artifacts:
 ...
 /tmp/hello/getting-started/target/getting-started-1.0.0-SNAPSHOT-native-image-source-jar/getting-started-1.0.0-SNAPSHOT-runner-build-output-stats.json (json)
 /tmp/hello/getting-started/target/getting-started-1.0.0-SNAPSHOT-native-image-source-jar/getting-started-1.0.0-SNAPSHOT-runner-timing-stats.json (raw)
 /tmp/hello/getting-started/target/getting-started-1.0.0-SNAPSHOT-native-image-source-jar/getting-started-1.0.0-SNAPSHOT-runner.build_artifacts.txt (txt)
========================================================================================================================
Finished generating 'getting-started-1.0.0-SNAPSHOT-runner' in 2m 53s.
[INFO] [io.quarkus.deployment.pkg.steps.NativeImageBuildRunner] objcopy --strip-debug getting-started-1.0.0-SNAPSHOT-runner
[INFO] [io.quarkus.deployment.QuarkusAugmentor] Quarkus augmentation completed in 177714ms
[INFO] ------------------------------------------------------------------------
[INFO] BUILD SUCCESS
[INFO] ------------------------------------------------------------------------
[INFO] Total time:  03:02 min
[INFO] Finished at: 2023-10-25T21:12:41Z
[INFO] ------------------------------------------------------------------------
hello $ 
----
...
ELF Header:
  ...
  Start of section headers:          43460616 (bytes into file)
  Flags:                             0x0
...
Since our environment here is Linux, you can just run it. In the VS Code Terminal, run:
...
[WARNING]
====
If you still have a previous application running in a separate VS Code Terminal, you may get an error like `java.net.BindException: Address already in use`. Go to that other tab and press kbd:[CTRL+C] to stop the previous application, then try to run the native application again!
====
...
INFO  [io.quarkus] (main) getting-started 1.0.0-SNAPSHOT native (powered by Quarkus 3.2.6.Final-redhat-00002) started in 0.026s. Listening on: http://0.0.0.0:8080
INFO  [io.quarkus] (main) Profile prod activated. 
INFO  [io.quarkus] (main) Installed features: [cdi, resteasy-reactive, smallrye-context-propagation, vertx]
----

That’s *26 milliseconds* to start up. The start-up time might be different in your environment.
...
----
    PID   RSS COMMAND
  16157 45420 /tmp/hello/getting-started/target/getting-started-1.0.0-SNAPSHOT-runner
----

This shows that our process is taking around *45MB* of memory (https://en.wikipedia.org/wiki/Resident_set_size[Resident Set Size^], or RSS). Pretty compact!
...
Make sure the app works. In a new VS Code Terminal run:
...
own OpenShift cluster, follow https://access.redhat.com/documentation/en-us/openshift_container_platform/4.14/html/serverless/install#install-serverless-operator[Installing OpenShift Serverless^].
...
[source,sh,role="copypaste"]
----
oc delete deployment/payment route/payment svc/payment
----
...
# quarkus.container-image.group={{ USER_ID }}-cloudnativeapps
...
Rebuild and re-deploy new payment service via running the following maven plugin in VS Code Terminal:

[source,sh,role="copypaste"]
----
mvn clean package -Pnative -DskipTests -f $PROJECT_SOURCE/payment-service
----
...
oc create -f $PROJECT_SOURCE/payment-service/knative/pipeline/apply_manifests_task.yaml && oc create -f $PROJECT_SOURCE/payment-service/knative/pipeline/update_deployment_task.yaml
----

Let’s confirm if the *tasks* are installed properly using https://github.com/tektoncd/cli/releases[Tekton CLI^] that already installed in VS Code:
...
buildah-1-12-0              Buildah task builds...   1 week ago
----
...
Now that the pipeline is created, check the list of pipelines you have created using the _Tekton CLI_ in VS Code Terminal:
...
[source,sh,role="copypaste"]
----
oc create -f $PROJECT_SOURCE/payment-service/knative/pipeline/persistent_volume_claim.yaml
----
