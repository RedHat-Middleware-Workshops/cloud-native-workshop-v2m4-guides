...
Create a *NetworkPolicy* to access normal, non-istioified sidecar applications(i.e. inventory). Back on the {{ CONSOLE_URL }}/topology/ns/{{ USER_ID }}-cloudnativeapps[Topology View, window=_blank], click on `+` icon on the right top corner:

image::plus-icon.png[serverless, 700]

Copy the following `NetworkPolicy` in `YAML` editor then click on *Create*:
...
image::pipeline-start.png[serverless, 700]
...
image::pipeline-complete.png[serverless, 700]
...
image::petclinic-openurl.png[serverless, 700]
